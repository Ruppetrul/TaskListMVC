...
class task_controller extends Task_core {
    ...
    function taskAction() {
        if (isset($_POST['change_status'])) {
            $this->m->alterTaskStatus(htmlspecialchars($_POST['change_status']));
        } else if (isset($_POST['delete'])) {
            $this->m->removeTask(htmlspecialchars($_POST['delete']));
        }
    ...
    function addNewTask() {
        if (isset($_POST['new_task']) && $_SESSION['user_id']) {
            $this->m->addTask($_SESSION['user_id'],htmlspecialchars($_POST['new_task']));
        }
        header("Location: index.php");
    }
}